...
{
    internal sealed class Lua : BaseApplication
    {
        public override string Name => "Lua";

        public Lua()
        {
            appPath = Path.Combine(BaseApplication.LocalApplicationData, "apps", "lua");
        ...
        public override bool Install(string version, IProgress<InstallProgress>? progress = null)
        {
            ...
            {
                case "5.5.0":
                    url = "https://github.com/minhnguyenerp/devkit2/releases/download/bin1.0.1/lua-5.5.0_Win64_bin.zip";
                    file = Path.Combine(Path.GetTempPath(), "lua-5.5.0_Win64_bin.zip");
                    break;
            }
            ...
            if (url != string.Empty && file != string.Empty)
            {
                ...
                File.WriteAllText(Path.Combine(appPath, version, "lua.bat"),
@"@echo off
lua55.exe %*");
                try
                {
