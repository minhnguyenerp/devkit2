...
{
    internal sealed class Codelite : BaseApplication
    {
        public override string Name => "Codelite";

        public Codelite()
        {
            appPath = Path.Combine(BaseApplication.LocalApplicationData, "apps", "codelite");
            ...
            get
            {
                if (_icon == null)
                {
                    if (InstalledVersions.Length > 0)
                    {
                        try
                        {
                            _icon = Icon.ExtractAssociatedIcon(Path.Combine(appPath, InstalledVersions[InstalledVersions.Length - 1].Value, "codelite.exe"));
                        }
                        catch { }
                    }
                }
